<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="物流单号L" prop="waybillL">
        <el-input
          v-model="queryParams.waybillL"
          placeholder="请输入物流单号L"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="relList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="物流单号L" align="center" prop="waybillL" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="物流单号L" prop="waybillL">
          <el-input v-model="form.waybillL" placeholder="请输入物流单号L" />
        </el-form-item>
        <el-form-item label="创建人" prop="createUser">
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.waybill)
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加物流单号和L关联";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const waybill = row.waybill || this.ids
      getRel(waybill).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改物流单号和L关联";
      });
    },
    /** 提交按钮 */
    submitForm() {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const waybills = row.waybill || this.ids;
      this.$modal.confirm('是否确认删除物流单号和L关联编号为"' + waybills + '"的数据项？').then(function() {
        return delRel(waybills);
      }).then(() => {
        this.getList();
